...
Looking into making species group columns (moved to dataRead-bats.R)
...
selectedGroup <- 'obligate'
selectedSpecies <- non_cave_obligate
...
if(selectedGroup == 'obligate'){ bats.sub <- bats.sub %>% mutate(groupby = obligate) }
...
# plotly using hovertemplate to zoom in on graph -----
...
if(input.grouping=='species'){ 
  bats.sub <- bats.sub %>% mutate( group=AUTO.ID, common=Common ) }
if(input.grouping=='species group'){ 
  bats.sub<-bats.sub %>% mutate( group=species_group, common=group_common) }
if(input.grouping=='cave dependency'){ 
  bats.sub <- bats.sub %>% mutate( group=obligate, common=obligate ) }
...
```{r}
# set wrap var column -----
if(yearly.wrapVar=='management'){ bats.yr <- bats.sub %>% 
  mutate(wrapV=habitat) %>% 
...
if(yearly.wrapVar == 'none'){
  ...
  bats.yr <- bats.yr %>% 
    ...
    mutate( nSensors = length(unique(siteID)) )
  bats.yr <- bats.yr %>% 
    group_by(year,wrapV,group,common) %>% 
    summarize( count=n(), relFreq=count/nSensors )
  bats.yr <- distinct(bats.yr)
}
```
...
```{r}
# make base plot -----
yearly.p <- ggplot( bats.yr ) +
  geom_line( aes(x=year, y=relFreq, color=common, text=paste("Group:",common)) ) +
  scale_fill_manual(values = cbPalette) +
